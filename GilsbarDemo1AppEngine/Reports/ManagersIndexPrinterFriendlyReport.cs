...
{
    class ManagersIndexPrinterFriendlyReport
    {
        public static void Generate(Report report, string fileSaveDirectory, Application app)
        {
            ...
            {
                string message = AppCommon.AppendInnerExceptionMessages("ManagersIndexViewReport.Generate: " + e.Message, e);
        ...
        private static void AddDocumentBody(Document document)
        {
            ...
            List<Manager> managersWebData = new List<Manager>();
            managersWebData = Web_Data.ManagersWebData.GetManagers();
            ...
            // add table data rows
            foreach (Manager manager in managersWebData)
            {
                table.Rows.Add();
                // format data row
                ...
                table.Rows[table.Rows.Count].Cells[1].Range.Text = manager.Firstname.ToString();
                table.Rows[table.Rows.Count].Cells[2].Range.Text = manager.Lastname.ToString();
                table.Rows[table.Rows.Count].Cells[3].Range.Text = manager.Email.ToString();
                table.Rows[table.Rows.Count].Cells[4].Range.Text = manager.HomePhone.ToString();
                table.Rows[table.Rows.Count].Cells[5].Range.Text = manager.CellPhone.ToString();
                table.Rows[table.Rows.Count].Cells[6].Range.Text = manager.WorkPhone.ToString();
                table.Rows[table.Rows.Count].Cells[7].Range.Text = manager.DateOfBirth.ToString();
